...
import { HEIGHT_OPTIONS } from "@/util/constants";
...
export function NameCard() {
  const { basic, nameCard, education, location } = useUserProfile().userProfile;
  const t = useTranslations();
  const height = HEIGHT_OPTIONS.find((h) => h.value === basic.height)?.label;
  const educationText = education.education
    ? Object.values(education.education)
        .sort((a, b) => a.educationOrder - b.educationOrder)
        .map((e) => t(`education.${e.degree}`).split("-")[0])
        .join(", ")
    : "";
      ...
      <Card className="lg:col-span-3 flex flex-col h-full">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Typography variant={"h3"}>{basic.name}</Typography>
          </div>
          ...
          <div className="flex items-center gap-3">
            <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
              <Button variant="outline">
                <IconUserScan />
                <span className="hidden md:inline">{t("profile.preview")}</span>
              </Button>
            </motion.div>
          </div>
        </CardHeader>
